...
void DrawInv()
{
	...
	if (plr[myplr].InvBody[INVLOC_HEAD]._itype != ITYPE_NONE) {
		...
		if (pcursinvitem == INVITEM_HEAD) {
			...
			CelDecodeClr(colour, 517, 219, (BYTE *)pCursCels, frame, frame_width);
		}

		if (plr[myplr].InvBody[INVLOC_HEAD]._iStatFlag) {
			CelDrawHdrOnly(517, 219, (BYTE *)pCursCels, frame, frame_width);
		} else {
			CelDrawHdrLightRed(517, 219, (BYTE *)pCursCels, frame, frame_width, 1);
		}
	}
	...
	if (plr[myplr].InvBody[INVLOC_RING_LEFT]._itype != ITYPE_NONE) {
		...
		if (pcursinvitem == INVITEM_RING_LEFT) {
			...
			CelDecodeClr(colour, 432, 365, (BYTE *)pCursCels, frame, frame_width);
		}

		if (plr[myplr].InvBody[INVLOC_RING_LEFT]._iStatFlag) {
			CelDrawHdrOnly(432, 365, (BYTE *)pCursCels, frame, frame_width);
		} else {
			CelDrawHdrLightRed(432, 365, (BYTE *)pCursCels, frame, frame_width, 1);
		}
	}
	...
	if (plr[myplr].InvBody[INVLOC_RING_RIGHT]._itype != ITYPE_NONE) {
		...
		if (pcursinvitem == INVITEM_RING_RIGHT) {
			...
			CelDecodeClr(colour, 633, 365, (BYTE *)pCursCels, frame, frame_width);
		}

		if (plr[myplr].InvBody[INVLOC_RING_RIGHT]._iStatFlag) {
			CelDrawHdrOnly(633, 365, (BYTE *)pCursCels, frame, frame_width);
		} else {
			CelDrawHdrLightRed(633, 365, (BYTE *)pCursCels, frame, frame_width, 1);
		}
	}
	...
	if (plr[myplr].InvBody[INVLOC_AMULET]._itype != ITYPE_NONE) {
		...
		if (pcursinvitem == INVITEM_AMULET) {
			...
			CelDecodeClr(colour, 589, 220, (BYTE *)pCursCels, frame, frame_width);
		}

		if (plr[myplr].InvBody[INVLOC_AMULET]._iStatFlag) {
			CelDrawHdrOnly(589, 220, (BYTE *)pCursCels, frame, frame_width);
		} else {
			CelDrawHdrLightRed(589, 220, (BYTE *)pCursCels, frame, frame_width, 1);
		}
	}
	...
	if (plr[myplr].InvBody[INVLOC_CHEST]._itype != ITYPE_NONE) {
		...
		if (pcursinvitem == INVITEM_CHEST) {
			...
			CelDecodeClr(colour, 517, 320, (BYTE *)pCursCels, frame, frame_width);
		}

		if (plr[myplr].InvBody[INVLOC_CHEST]._iStatFlag) {
			CelDrawHdrOnly(517, 320, (BYTE *)pCursCels, frame, frame_width);
		} else {
			CelDrawHdrLightRed(517, 320, (BYTE *)pCursCels, frame, frame_width, 1);
		}
	}
